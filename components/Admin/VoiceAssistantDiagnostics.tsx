...
export const VoiceAssistantDiagnostics: React.FC = () => {
  ...
  const runDiagnostics = async () => {
    try {
      ...
      // Transform response data to ensure it matches our expected format
      const data = response.data;
      
      // If the API still returns vapiConfig, map it to voiceConfig
      if (data.vapiConfig && !data.voiceConfig) {
        data.voiceConfig = data.vapiConfig;
        delete data.vapiConfig;
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">Voice Assistant Diagnostics</h2>
        <button
      ...
      {diagnostics && (
        <div className="bg-white rounded-lg shadow overflow-hidden">
          ...
          <div className="p-4">
            ...
            {/* Voice Assistant Config Status */}
            <div className="mb-6">
              <h3 className="font-medium text-lg mb-2">Voice Assistant Configuration</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span className="text-gray-700">Server API Key:</span>
                    {getConfigStatusIndicator(diagnostics.voiceConfig.serverApiKeyConfigured)}
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-700">Client API Key:</span>
                    {getConfigStatusIndicator(diagnostics.voiceConfig.clientApiKeyConfigured)}
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-700">Default Assistant ID:</span>
                    {getConfigStatusIndicator(diagnostics.voiceConfig.assistantIdConfigured)}
                  </div>
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span className="text-gray-700">Webhook Secret:</span>
                    {getConfigStatusIndicator(diagnostics.voiceConfig.webhookSecretConfigured)}
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-700">Base URL:</span>
                    {getConfigStatusIndicator(diagnostics.voiceConfig.baseUrlConfigured)}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 p-4 bg-blue-50 border border-blue-100 rounded-md">
              <div className="flex items-start">
                ...
                <div>
                  <h4 className="font-medium text-blue-700">Need help with troubleshooting?</h4>
                  <p className="mt-1 text-sm">
                    Check our troubleshooting guide for common solutions to configuration issues.
                  </p>
                  <div className="mt-2">
                    <a 
                      href="/docs/voice-troubleshooting.md" 
      ...
      {!diagnostics && !loading && !error && (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          <h3 className="text-xl font-semibold mb-2">No Diagnostic Data</h3>
          <p className="text-gray-600 max-w-md mx-auto mb-6">
            Click the "Run Diagnostics" button to check your voice assistant configuration and MongoDB connection.
          </p>
          <button
            onClick={runDiagnostics}
